...
export const petalForm = (form: Genome["form"], rng: Rng): PetalForm => {
...
const kernel = (t: number, { a, b }: Flank): number => {
  const peak = a / (a + b);
  return (t ** a * (1 - t) ** b) / (peak ** a * (1 - peak) ** b);
};

const aligned = (f: PetalForm, x: number, y: number): [number, number] => {
  const cos = Math.cos(-f.lean);
  const sin = Math.sin(-f.lean);
  return [x * cos - y * sin, x * sin + y * cos];
};

export const midrib = (f: PetalForm, t: number): [number, number] =>
  aligned(f, f.bend * f.length * t * t, -f.length * t);

const normal = (f: PetalForm, t: number): [number, number] => {
...
const ripple = (f: PetalForm, flank: Flank, t: number): number =>
  f.wave *
...
const closedPath = (points: [number, number][]): string => {
...
export const petalOutline = (f: PetalForm, samples = 20): string => {
...
export const midribPath = (f: PetalForm): string => {
...
export const tipLean = (f: PetalForm): number => {
