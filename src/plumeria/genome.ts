...
const pickCultivar = (rng: Rng): Recipe => {
...
const sampleTone = (rng: Rng, span: ToneSpan): Tone => ({
  c: between(rng, ...span.c),
  h: between(rng, ...span.h),
  l: between(rng, ...span.l),
});

const samplePalette = (rng: Rng, recipe: Recipe): Palette => ({
  blush: {
...
// pale and silvery.
export const sampleGenome = (rng: Rng, moon = 0): Genome => {
